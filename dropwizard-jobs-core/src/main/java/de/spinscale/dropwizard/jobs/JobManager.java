...
//	}
	...
	public void configure(JobConfiguration config) {
	...
	private void createOrUpdateJob(JobKey jobKey, Class<? extends org.quartz.Job> clazz, Trigger trigger,
		...
		if (!scheduler.checkExists(jobKey)) {
			...
			// from creating multiple instances of the same job when running in
			...
			if (triggers.size() == 1) {
				...
				// since we want to enforce a one-to-one relationship between
				// jobs and triggers
